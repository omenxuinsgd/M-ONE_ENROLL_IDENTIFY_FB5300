...
def compress_raw(image_data, width, height, dpi=500, bpp=8, compression_alg=DPFJ_COMPRESSION_WSQ_NIST, bitrate_x100=None, target_size=None, tolerance_aw=10):
    """
    Compress raw fingerprint image using WSQ algorithm.
    :return: Compressed image data (bytes) or None if failed.
    """
    # Log parameter values for debugging
    print(f"[DEBUG] Compression parameters:")
    print(f"  - Width: {width}")
    print(f"  - Height: {height}")
    print(f"  - DPI: {dpi}")
    print(f"  - BPP: {bpp}")
    print(f"  - Compression algorithm: {compression_alg}")
    print(f"  - Bitrate_x100: {bitrate_x100}")
    print(f"  - Target size: {target_size}")
    print(f"  - Tolerance AW: {tolerance_aw}")

    # Validate parameters
    if not validate_compression_parameters(image_data, width, height, dpi, bpp):
        return None

    # Convert image_data to c_ubyte array
    image_array = (ctypes.c_ubyte * len(image_data)).from_buffer_copy(image_data)

    # Step 1: Start compression
    result = dpfj_compression_dll.dpfj_start_compression()
    if result != DPFJ_SUCCESS:
        print(f"[ERROR] Failed to start compression. Error Code: {result}")
        logging.error(f"Failed to start compression. Error Code: {result}")
        return None

    # Step 2: Set bitrate or target size
    if bitrate_x100 is not None:
        # Use dpfj_set_wsq_bitrate
        result = dpfj_compression_dll.dpfj_set_wsq_bitrate(bitrate_x100, tolerance_aw)
        if result != DPFJ_SUCCESS:
            print(f"[ERROR] Failed to set bitrate. Error Code: {result}")
            dpfj_compression_dll.dpfj_finish_compression()  # Clean up resources
            return None
        print(f"[DEBUG] Bitrate set to {bitrate_x100 / 100} bpp.")
        logging.debug(f"Bitrate set to {bitrate_x100 / 100} bpp.")
    elif target_size is not None:
        # Use dpfj_set_wsq_size
        result = dpfj_compression_dll.dpfj_set_wsq_size(target_size, tolerance_aw)
        if result != DPFJ_SUCCESS:
            print(f"[ERROR] Failed to set target size. Error Code: {result}")
            dpfj_compression_dll.dpfj_finish_compression()  # Clean up resources
            return None
        print(f"[DEBUG] Target size set to {target_size} bytes.")
        logging.debug(f"Target size set to {target_size} bytes.")
    else:
        print("[ERROR] Either bitrate_x100 or target_size must be specified.")
        dpfj_compression_dll.dpfj_finish_compression()  # Clean up resources
        return None

    # Step 3: Compress raw image
    result = dpfj_compression_dll.dpfj_compress_raw(
    ...
    if result != DPFJ_SUCCESS:
        print(f"[ERROR] Failed to compress raw image. Error Code: {result}")
        logging.error(f"Failed to compress raw image. Error Code: {result}")
        dpfj_compression_dll.dpfj_finish_compression()  # Clean up resources
        return None

    # Step 4: Get processed data (compressed image)
    compressed_data = get_processed_data()
    if not compressed_data:
        print("[ERROR] Failed to retrieve compressed data.")
        logging.error("Failed to retrieve compressed data.")
        dpfj_compression_dll.dpfj_finish_compression()  # Clean up resources
        return None
...
def enroll_user(dev, user_id, num_scans=4):
    ...
    for i in range(num_scans):
        ...
        # Extract raw image data (if necessary)
        raw_image_data = extract_raw_image(image_data)
        ...
        # Contoh penggunaan dengan berbagai bitrate
        bitrates = [75, 80, 85, 90]  # 0.75 bpp, 0.80 bpp, 0.85 bpp, 0.90 bpp
        for bitrate in bitrates:
            compressed_data = compress_raw(
                ...
                compression_alg=DPFJ_COMPRESSION_WSQ_NIST,  # Compression algorithm
                bitrate_x100=bitrate,  # Bitrate (e.g., 75 for 0.75 bpp)
                tolerance_aw=10  # Tolerance for Aware WSQ
            )
            if compressed_data:
                save_wsq_file(compressed_data, f"compressed_{bitrate}.wsq")
                print(f"Compressed image saved with bitrate {bitrate}.")
                # Contoh penggunaan dengan target size
                compressed_data = compress_raw(
                    raw_image_data,  # Raw image data
                    width=400,       # Image width
                    height=500,      # Image height
                    ...
                    bpp=8,           # Bits per pixel
                    compression_alg=DPFJ_COMPRESSION_WSQ_NIST,  # Compression algorithm
                    target_size=10240,  # Target size (10 KB)
                    tolerance_aw=10     # Tolerance for Aware WSQ
                )
        if compressed_data:
            save_wsq_file(compressed_data, "compressed_target_size_10KB.wsq")
            print("Compressed image saved with target size 10 KB.")
...
def extract_raw_image(image_data, width=400, height=500):
    ...
    """
    # Contoh: Jika header ISO/ANSI memiliki panjang 100 byte, ekstrak data mulai dari byte ke-100
    # Hitung ukuran data gambar yang diharapkan
    expected_size = width * height

    # Cari posisi awal data gambar mentah
    # Header ISO/ANSI biasanya memiliki panjang tertentu (misalnya, 100 byte)
    ...
    if len(image_data) <= header_size:
        print("[ERROR] Image data is too small to contain a valid header.")
        logging.error("Image data is too small to contain a valid header.")
        return None
    ...
    logging.debug(f"Raw image data size: {len(raw_image_data)} bytes")
    return raw_image_data

def expand_raw(compressed_data, compression_alg=DPFJ_COMPRESSION_WSQ_NIST):
    """
    Expand (decompress) compressed raw image using WSQ algorithm.

    :param compressed_data: The compressed image data (bytes).
    :param compression_alg: The compression algorithm (default: NIST WSQ).
    :return: A tuple containing the decompressed image data and its metadata (width, height, dpi, bpp), or None if failed.
    """
    # Start compression (decompression uses the same API)
    result = dpfj_compression_dll.dpfj_start_compression()
    if result != DPFJ_SUCCESS:
        print(f"[ERROR] Failed to start decompression. Error Code: {result}")
        return None

    # Expand raw image
    compressed_array = (ctypes.c_ubyte * len(compressed_data)).from_buffer_copy(compressed_data)
    image_width = ctypes.c_uint(0)
    image_height = ctypes.c_uint(0)
    image_dpi = ctypes.c_uint(0)
    image_bpp = ctypes.c_uint(0)

    result = dpfj_compression_dll.dpfj_expand_raw(
        compressed_array,  # image_data
        len(compressed_data),  # image_size
        compression_alg,  # compression_alg
        ctypes.byref(image_width),  # image_width
        ctypes.byref(image_height),  # image_height
        ctypes.byref(image_dpi),  # image_dpi
        ctypes.byref(image_bpp)  # image_bpp
    )
    if result != DPFJ_SUCCESS:
        print(f"[ERROR] Failed to expand raw image. Error Code: {result}")
        dpfj_compression_dll.dpfj_finish_compression()
        return None

    # Get processed data (decompressed image)
    decompressed_data = get_processed_data()
    if not decompressed_data:
        print("[ERROR] Failed to retrieve decompressed data.")
        dpfj_compression_dll.dpfj_finish_compression()
        return None
    ...
    # Return decompressed data and metadata
    return {
