...
def capture_fingerprint(dev, timeout=5000, image_format=DPFPDD_IMG_FMT_ISOIEC19794, image_proc=DPFPDD_IMG_PROC_ENHANCED, image_res=500):
    ...
    if result == 0:
        ...
        # Debug: Print raw data (first 100 bytes)
        print(f"[DEBUG] First 100 bytes of raw data: {image_buffer.raw[:100]}")
        ...
        print(f"Failed to capture fingerprint. Error Code: {result}")
        return None
