...
def enroll_user(dev, user_id, num_scans=4):
    ...
    for i in range(num_scans):
        ...
        # Skip image quality validation for now
...
if __name__ == "__main__":
    ...
    if identified_user:
        ...
        # Load the enrolled FMD for diagnostics
        with open(f"fmd_{identified_user}.dat", "rb") as f:
            enrolled_fmd = f.read()

        # Capture a new fingerprint for diagnostics
        print("Capture a new fingerprint for diagnostics...")
        new_image_data = capture_fingerprint(dev)
        if new_image_data:
            new_fmd = create_fmd_from_raw(
            ...
            if new_fmd:
                ...
                diagnostic_compare(new_fmd, enrolled_fmd)
