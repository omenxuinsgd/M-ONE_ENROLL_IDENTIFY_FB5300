...
# Define dpfj_dp_fid_convert function
dpfj_dll.dpfj_dp_fid_convert.argtypes = [
    ctypes.POINTER(ctypes.c_ubyte),  # dp_image
    ctypes.c_uint,                   # dp_image_size
    DPFJ_FID_FORMAT,                 # fid_type
    ctypes.c_uint,                   # fid_dpi
    ctypes.c_uint,                   # rotate180
    ctypes.POINTER(ctypes.c_ubyte),  # fid
    ctypes.POINTER(ctypes.c_uint)    # fid_size
]
dpfj_dll.dpfj_dp_fid_convert.restype = ctypes.c_int
...
def convert_fmd(fmd1, fmd1_size, fmd1_type, fmd2_type):
    """
    Convert FMD from one format to another.

    :param fmd1: The source FMD (bytes).
    :param fmd1_size: The size of the source FMD.
    :param fmd1_type: The type of the source FMD (e.g., DPFJ_FMD_ANSI_378_2004).
    :param fmd2_type: The target FMD format (e.g., DPFJ_FMD_ISO_19794_2_2005).
    :return: The converted FMD (bytes) or None if failed.
    """
    # Convert fmd1 to c_ubyte array
    fmd1_array = (ctypes.c_ubyte * fmd1_size).from_buffer_copy(fmd1)

    # Initialize fmd2 buffer and size
    fmd2 = (ctypes.c_ubyte * 65536)()  # Buffer for converted FMD (max size)
    fmd2_size = ctypes.c_uint(65536)   # Buffer size

    # Call dpfj_fmd_convert
    result = dpfj_dll.dpfj_fmd_convert(
        fmd1_type,      # fmd1_type
        fmd1_array,     # fmd1
        fmd1_size,      # fmd1_size
        fmd2_type,      # fmd2_type
        fmd2,           # fmd2
        byref(fmd2_size)  # fmd2_size
    )

    if result == DPFJ_SUCCESS:
        print("FMD converted successfully.")
        return bytes(fmd2[:fmd2_size.value])
    elif result == DPFJ_E_INVALID_PARAMETER:
        print("[ERROR] Invalid parameter passed to dpfj_fmd_convert.")
        return None
    else:
        print(f"[ERROR] Failed to convert FMD. Error Code: {result}")
        return None
...
def compare_fingerprints(fmd1, fmd1_view_idx, fmd2, fmd2_view_idx):
    """
    Compare two fingerprints using their FMDs.

    :param fmd1: The first FMD (bytes).
    :param fmd1_view_idx: The view index in the first FMD.
    :param fmd2: The second FMD (bytes).
    :param fmd2_view_idx: The view index in the second FMD.
    ...
    if result == DPFJ_SUCCESS:
        ...
        print(f"[ERROR] Failed to compare fingerprints. Error Code: {result}")
        return None
        ...
        if new_fmd:
            ...
            if candidates:
                for i, candidate in enumerate(candidates):
                    ...
                    # Compare the new FMD with the enrolled FMD
                    print("Comparing fingerprints...")
                    score = compare_fingerprints(new_fmd, 0, enrolled_fmds[candidate.fmd_idx], candidate.view_idx)
                    if score is not None:
                        if score == 0:
                            print("Fingerprints match perfectly.")
                        else:
                            print(f"Fingerprints do not match. Dissimilarity score: {score}")
